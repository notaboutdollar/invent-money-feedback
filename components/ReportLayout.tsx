import type {
  BarQuestion as BarQuestionData,
  RatingQuestion as RatingQuestionData,
  ThemeQuestion as ThemeQuestionData,
  SummaryQuestion as SummaryQuestionData,
  BarRow,
} from "@/data/report-types";

// ————————————————————————————————————————————————————————————————
// Question renderers shared by every editorial report page
// ————————————————————————————————————————————————————————————————
export function BarQuestion({
  q,
}: {
  q: BarQuestionData & { readingLabel: string };
}) {
  return (
...
export function BarList({ bars }: { bars: BarRow[] }) {
  return (
    <div className="bars">
      {bars.map((b, i) => (
...
export function RatingQuestion({
  ...
  q: RatingQuestionData & { readingLabel: string };
}) {
                  ...
                  return (
                    <div className="rating-col" key={i}>
                      <div
                        ...
                        style={
                          !isZero ? { height: `${heightPct}%` } : undefined
                        }
                      />
                      <div className="col-lbl">
                        <span>{i + 1}</span>
                        <span className={`n${isZero ? " zero-c" : ""}`}>
                          {c}
                        </span>
                      </div>
                    </div>
...
export function SummaryQuestion({ q }: { q: SummaryQuestionData }) {
  return (
    <section id={`q${q.num}`} className={q.zebra ? "zebra" : ""}>
      <div className="wrap">
        <div className="q-head">
...
export function Reading({ text, label }: { text: string; label: string }) {
  return (
...
// ————————————————————————————————————————————————————————————————
// CSS shared by every editorial report — scoped under .report-page
// ————————————————————————————————————————————————————————————————
...
.report-page .toc-older-item { margin-top: 2px; }
